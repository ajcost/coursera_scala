...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = b.isStanding && b.b1 == b.b2 && b.b1 == goal
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    lazy val l = b.legalNeighbors map { case (block, move) => (block, move :: history) }
    l.toStream
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (b, m) #:: s =>
      val newNeighbors = newNeighborsOnly(neighborsWithHistory(b, m), explored + b)
      newNeighbors ++ from(s ++ newNeighbors, explored + b)
  }
