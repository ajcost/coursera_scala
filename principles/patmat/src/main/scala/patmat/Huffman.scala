...
object Huffman {
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def recurDecode(subtree: CodeTree, bits: List[Bit]): List[Char] = subtree match {
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def recurEncode(tree:CodeTree)(ch: Char): List[Bit] = tree match {
        case Leaf(_, _) => Nil
        case Fork(l, r, c, _) => if (chars(l).contains(ch)) 0 :: recurEncode(l)(ch) else 1 :: recurEncode(r)(ch)
      }
      text.flatMap(f => recurEncode(tree)(f))
    }
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = table.filter(c => c._1 == char).head._2
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def quickEncodeRec(table: CodeTable, text: List[Char]): List[Bit] = {
        if (text.isEmpty) Nil else codeBits(table)(text.head) ::: quickEncodeRec(table, text.tail)
      }
      quickEncodeRec(convert(tree), text)
    }
  }
